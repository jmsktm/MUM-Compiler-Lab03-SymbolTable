...
public class Main {
	...
	static void enterScope(String name, int line) {
		...
		if (se1 != null) {
			if (se1 instanceof ScopeEntry) {
				...
				System.out.println("Successful command " + line + ": new_scope " + name);
			} else {
				System.out
						.println("Unsuccessful command " + line + ": new_scope " + name + " -- not a class or method");
			}
		} else {
			System.out.println("Unsuccessful command " + line + ": new_scope " + name + " -- not a class or method");
		}
	}
	...
	static void errorMessage(Yytoken tok[], int length, String msg) {
		System.out.print("Unsuccessful command " + tok[0].m_line + ": ");
		System.out.print(tok[0].m_text + " ");
		for (int i = 1; i < length; i++) {
			System.out.print(" " + tok[i].m_text);
		}
	...
	static void invalidCmdMessage(Yytoken tok[], int length, String msg) {
		System.out.print("Invalid command " + tok[0].m_line + ": ");
		System.out.print(tok[0].m_text);
		for (int i = 1; i < length; i++) {
			System.out.print(" " + tok[i].m_text);
		}
		...
		while (token[0] != null) {
			...
			case Yytoken.t_method: {
				...
				if (token[3].m_index != Yytoken.t_l_par) {
					errorMessage(token, 4, "expecting '(' but found " + token[3].m_text);
					break;
				}
				...
				while (token[4].m_index != Yytoken.t_r_par) {
					...
					if (parmtype == null) {
						errorMessage(token, 5,
								" -- invalid parameter type" + "expecting a type but found " + token[4].m_text);
						break;
					}
					...
					if (parm1 == null) {
						errorMessage(token, 6,
								" -- invalid parameter name" + "expecting an identifier but found " + token[5].m_text);
						break;
					}
					if (!symtab.insertBinding(parm1)) {
						errorMessage(token, 6, " -- duplicate parameter names " + token[5].m_text);
					}
					token[4] = nextToken(lexer);
				...
				se1 = symtab.lookup(name);
				if (se1 != null) {
					errorMessage(token, 4, ") -- redeclaration of identifier " + name);
			...
			default: {
				System.out.print("Unknown command at line " + token[0].m_line + ": ");
